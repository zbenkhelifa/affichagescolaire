...
import { Monitor, Hash, School } from 'lucide-react';
...
export const MobileAuthScreen: React.FC<Props> = ({ onDisplayCode }) => {
  ...
  const handleCode = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!uai.trim() || !code.trim()) return;
    setLoading(true);
    setError('');
    try {
      const schoolId = await sb.lookupByDisplayCode(uai.trim(), code.trim());
      if (!schoolId) {
        ...
        >
          ...
          <form onSubmit={handleCode} className="p-5 space-y-4">
            <p className="text-white/50 text-xs leading-relaxed">
              Entrez le code UAI et le code d'affichage fournis par votre établissement.
            </p>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3.5 rounded-xl text-base text-white placeholder-white/25 focus:outline-none transition-colors font-mono tracking-wider"
            ...
            <button
              type="submit"
              disabled={loading || !uai.trim() || !code.trim()}
